...
class OrderControllerIntegrationTests {
    ...
    private void givenPaymentWillSucceed(Long orderId) {
        ...
        given(orderService.pay(eq(orderId), eq("4532756279624064"))).willReturn(payment);
    }
    ...
    private void givenOrderIsPaid(Long orderId) {
        given(orderService.pay(eq(orderId), any())).willThrow(OrderAlreadyPaid.class);
    }

    private Long givenReceipt() {
        Receipt receipt = new Receipt(LocalDateTime.now(), "4532756279624064", Money.of(100.0, "EUR"));
        given(orderService.getReceipt(eq(1L), eq(Monetary.getCurrency("EUR")))).willReturn(receipt);
        return 1L;
    }
}
